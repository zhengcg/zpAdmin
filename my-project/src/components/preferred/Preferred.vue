<template>
	<div>
	<ol class="breadcrumb">
	    <li><a href="javascript:;">正品优选</a></li>
		...
		<div class="panel panel-info">
		    <div class="panel-heading">		    
		        <h3 class="panel-title">
		            正品优选
		            <router-link class="pull-right addBtn" :to="{name:'addPre'}"><i class="glyphicon glyphicon-plus"></i>添加</router-link>
		        </h3>
		    </div>
				  ...
				  <table class="table table-condensed table-bordered table-striped">
				    ...
				    <tbody>
				    	<tr v-for="item in list">
				    		...
				    		<!-- <td><img :src="item.thumbnail" alt="缩略图"></td> -->
				    		...
				    		<td style="white-space:nowrap"><router-link class="btn btn-default" :to="{name:'updatePre', query: { id:item.id}}" style="margin-right:10px;">修改</router-link><router-link class="btn btn-primary" :to="{name:'findPre', query: { id:item.id}}" style="margin-right: 10px;">查看</router-link><a href="javascript:;" class="btn btn-danger" @click="removeFn(item.id)" style="margin-right: 10px;">删除</a><a :href="item.snapshot" target="_blank" class="btn btn-info" v-if="item.snapshot">预览</a></td>				    		
				    	</tr>
...
export default {
  ...
  data () {
    return {
      api:{
      	getList:"/genuine/server_preference/serverList",
      	remove:"/genuine/server_preference/serverDelete"
      },
      list:[],
      count:0,
      isInitPage:false
