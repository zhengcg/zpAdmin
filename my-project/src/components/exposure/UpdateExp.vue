		...
		<div class="panel-body">
    		...
    		<form class="form-horizontal" role="form">
						  ...
						  <div class="form-group">
						   ...
						   <div class="form-group">
						    <label for="newsType" class="col-sm-2 control-label"><span style="color: red">*</span>新闻类型</label>
						    <div class="col-sm-10">
						    <select class="form-control" id="newsType" name="newsType" v-model="model.newsType">
							      <option value="1">曝光</option>
							      <option value="2">行业资讯</option>
							      <option value="3">正品动态</option>							     
							   </select>
						      <!-- <input type="text" class="form-control" id="shortTitle" name="shortTitle" v-model="model.shortTitle" maxlength="60">
						      <p style="font-size: 12px;color: #999;">请输入少于60位的字符</p> -->
						    </div>
						  ...
						  <div class="form-group">
						  	<label for="hazardClass" class="col-sm-2 control-label">危害等级</label>
						    <div class="col-sm-10">
...
export default {
  data () {
    return {
      ...
      api:{
      	...
      	updateDate:"/genuine/server_exposureBar/serverUpdate",
  ...
  methods:{
	 ...
	 updateItem:function(){
		...
		self.updateData.newsType=self.model.newsType;
		        ...
		        self.axios.post(self.api.updateDate, self.updateData)
			            .then(response => {
			            	if(response.data.errorCode=="0"){
			           		zdalert('提示',response.data.reason,function(){
		            			// location.reload()
		            			self.$router.push({name:'exposure'})
		            		});          			                 	               
			            	}else{
			            		zdalert('提示',response.data.reason,function(){});
			            	}                        
